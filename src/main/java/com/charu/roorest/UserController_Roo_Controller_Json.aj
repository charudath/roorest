...
import com.charu.roorest.UserController;
...
privileged aspect UserController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> UserController.showJson(@PathVariable("id") UserPK id) {
    ...
    public ResponseEntity<String> UserController.deleteFromJson(@PathVariable("id") UserPK id) {
        User user = User.findUser(id);
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        if (user == null) {
